...
LANGUAGE_CODE_TO_INSTRUCTIONS: dict[LanguageCode | None, str] = {
    ...
    "en": "Speak only English. Do not switch to French even if the transcription looks like French or contains errors — if a phrase is ambiguous, interpret it as English.",
...
# Appended to the system prompt when a session has tools. Two jobs:
#   1. Latency cover. An OpenRouter round-trip is slow enough to leave dead air
#      mid-call, so the model is asked to speak a short lead-in BEFORE calling a
#      tool. That text streams to the TTS while the HTTP call is in flight.
#   2. Honesty. The backend owns truth: the model must report what the tool
#      actually returned rather than confirming something it merely attempted.
TOOL_USE_INSTRUCTIONS = """
# USING TOOLS
You can take real actions for the guest by calling tools.

Before you call a tool, say a short, natural lead-in out loud so the guest is
not left in silence, then make the call in the same turn. For example: "Sure,
let me book that for you now," or "One moment, I'll get that ordered." Keep it
to a few words and vary it; never say the tool's name.

Never say an action is done before the tool has told you it is done. When the
result comes back, tell the guest what it actually says: if it succeeded, give
them the details it returned; if it failed, say so plainly and offer the
alternative it suggests. Do not invent booking references, prices, driver
names, or times, and never guess a price - the hotel system calculates totals
and tells you what they are.

If you are missing something you need, such as the room number, ask the guest
