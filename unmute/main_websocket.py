...
CORS_ALLOW_ORIGINS = ["http://localhost", "http://localhost:3000"] + _extra
app.add_middleware(
    CORSMiddleware,
...
async def receive_loop(
    ...
    while True:
        ...
        elif isinstance(message, ora.UnmuteToolCallResult):
            handler.resolve_client_tool(message.call_id, message.result)

        elif isinstance(message, ora.UnmuteResponseCreate):
            await handler.create_response(message.note)
